#Code revised from https://github.com/mtli/HTML4Vision/blob/master/html4vision/thumbs.py
...
class ThumbnailGenerator:
    def __init__(self, thumbs_dir, imsize, imscale, preserve_aspect=True, quality=95):
        self.thumbs_dir = thumbs_dir #the root to save thumb files
        self.imsize = imsize
    ...
    def make_thumb(self, filename):
        thumb_filename = os.path.join(self.thumbs_dir, filename)
        os.makedirs(os.path.dirname(thumb_filename), exist_ok=True) #make dirs for thumb files
        thumb_ok = True
        st_filename = os.stat(filename)
        #check whether need to recompute the file for updating
        try:
